...
void Update() {
	...
	VehicleState vs; //instance of class struct created. Allows variables to be accessed from struct. 
	vs.x = vehicle->getX();
	vs.z = vehicle->getZ();
	vs.rotation = vehicle->getRotation();
	...
	//Below is defintions for Waypoints
	double x_length = goal.x - vs.x;
	double z_length = goal.z - vs.z;
	double distance = sqrt((x_length)*(x_length)+(z_length)*(z_length));
	//atan2(y, x), in math.h, considers what quadrants your inputs are in.
	//normal atan doesn't know quadrants, atan(y/x), if both are negative becomes positive again
	double relativeAngle = atan2(z_length, x_length) * (180 / PI);
	double angle = vs.rotation - relativeAngle;

	//getobstacles radius is obstacles radius 
	//iscolliding radius is car's bubble 
	std::list<Obstacle> obstacles = ObstacleManager::get()->getObstacles(vs.x, vs.z, 10);

	closestObstacle = 20; //set because this is never going to be true, my max is 10. 
	...
	//cout << "closest obstacle" << closestObstacle << endl; 
	//cout << "obstacle relative angle" << obstacleRelativeAngle << endl; 
	//cout << "obstacleAngle" << obstacleAngle << endl;

	//need to convert speed into a number with max 10
	//need to convert angle into a number with max 15
...
void ProgressUpdate() {
	...
	//goals are 0, 1, 2, 3, 4, 5 so 6 in total, goal.size() = 6. 
